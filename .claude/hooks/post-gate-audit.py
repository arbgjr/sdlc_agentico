...
class PostGateAuditor:
    ...
    def __init__(self, phase: int):
        ...
        self.logger = get_logger(__name__, skill="adversarial-validator", phase=phase)
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"adversarial_audit": {"enabled": True, "phases": list(range(9))}}
        except yaml.YAMLError as e:
            self.logger.error("Failed to parse config file", extra={"error": str(e)})
            return {"adversarial_audit": {"enabled": True, "phases": list(range(9))}}
    ...
    def should_audit(self, config: Dict[str, Any]) -> bool:
        ...
        if self.phase not in phases_to_audit:
            self.logger.info(
                ...
                extra={"phase": self.phase, "configured_phases": phases_to_audit},
            )
            return False
    ...
    def run_audit(self, project_path: str) -> Optional[Path]:
        ...
        try:
            self.logger.info(
                ...
                extra={"phase": self.phase, "project": project_path},
            )
            ...
            self.logger.info("Audit execution completed", extra={"phase": self.phase})
            ...
            self.logger.error(
                "Audit script timed out", extra={"phase": self.phase, "timeout": 300}
            )
            return None

        except Exception as e:
            self.logger.error("Unexpected error running audit", extra={"error": str(e)})
            return None
    ...
    def parse_audit_report(self, report_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                "Audit report not found", extra={"path": str(report_path)}
            )
            return None
        except yaml.YAMLError as e:
            self.logger.error("Failed to parse audit report", extra={"error": str(e)})
            return None

    def handle_decision(self, report: Dict[str, Any], config: Dict[str, Any]) -> int:
        """
        ...
        if decision == "FAIL":
            ...
            if auto_correct:
                ...
                self.logger.info(
                    "Attempting auto-correction", extra={"max_retries": max_retries}
                )
            ...
            self.logger.info(
                "Creating tech debt issues for findings", extra={"phase": self.phase}
            )
            ...
            self.logger.info(
                "Adversarial audit PASSED", extra={"phase": self.phase, "findings": 0}
            )
            return 0
            ...
            self.logger.error(
                ...
                extra={"decision": decision, "phase": self.phase},
            )
            return 1
